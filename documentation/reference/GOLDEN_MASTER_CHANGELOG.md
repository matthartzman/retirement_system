...
`documentation/archive/superpowers/specs/2026-09-02-monarch-autoupdate-reporting-design.md`
has the full design.
...
`documentation/reference/OPTIMIZATION_REFACTOR_STATUS.md`'s matching entry for why.
...
`documentation/reference/OPTIMIZATION_REFACTOR_STATUS.md`.
...
1. **The terminal cliff (H1) is exactly zero for the frozen fixture's beneficiary.** A spouse
   ...
   cliff/reporting change on this codebase (`documentation/archive/reports/PLANNER_SIGNOFF_2026-08-17.md`).
2. **`hsa_withdrawal_mode='optimize'` is not reachable from real plan data at all.**
   ...
   `documentation/archive/superpowers/plans/2026-08-17-hsa-withdrawal-optimizer.md` — a future task must admit
...
`regen --reason <file>`) and `documentation/reference/GOLDEN_MASTER_RECOVERY_RUNBOOK.md`, mechanizing the two
method traps recorded in the 2026-08-10 postmortem
(`documentation/archive/superpowers/plans/2026-08-10-golden-master-and-at-rest-plan-data-migration.md`, "Phase 1"):
...
**What changed.** Three follow-ups from `documentation/archive/reports/PLANNER_SIGNOFF_2026-08-17.md`. **No
...
(`documentation/archive/reports/PLANNER_SIGNOFF_2026-08-17.md` §5), which also confirmed C1's and C2's fixes
...
**Provenance.** Found by `documentation/archive/reports/PLANNER_SIGNOFF_2026-08-17.md` (finding S2), which
...
`documentation/reference/CLAUDE.md`'s "Golden master maintenance" regen snippet read
...
`documentation/archive/reports/SYSTEM_REVIEW_2026-08-04.md` §7) authorized proceeding with
...
- Added `documentation/archive/FLASK_REMOVAL_ARCHITECTURE.md`, proposing a dependency-free local HTTP runtime, transport-neutral request/response contracts, and a phased migration path that preserves existing `/api/...` URLs while removing Flask/Werkzeug from the packaged app.
- Added `documentation/archive/PLANNING_WORKBENCH_CONSOLIDATION_PROPOSAL.md`, rationalizing Build Impact, Scenarios, Stress Tests, and Strategy into one Planning Workbench model with Baseline, Change Set, Run Type, and Impact concepts.
