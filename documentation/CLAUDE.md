...
Plan data (client facts, income, spending, assets, etc.) lives in three mirrored forms:

1. **`input/client_*.csv`** — canonical on-disk working copy; always read first by `_read_plan_data_file()`
2. **`local_state/retirement_system_v10.db`** — SQLite with two relevant tables:
   - `settings` — flat key-value rows (section/subsection/label/value), updated by `import_csv_to_sqlite()`
   - `client_files` — stores raw CSV file content verbatim, updated by `set_client_file()`
3. **`input/client_*.yaml` and `input/client_*.json`** — derived outputs regenerated by `export_client_json_yaml()`

`_sync_config_backends()` runs after any plan data write: it calls `export_client_json_yaml()` (CSV → YAML/JSON) and `import_csv_to_sqlite()` (CSV → `settings` table).

**Critical**: `_read_plan_data_file()` checks the on-disk `input/` file first. If that file exists (even if blank from "Start New Plan"), the SQLite mirror is ignored. This means `client_files` in SQLite is only the fallback when the on-disk file is absent.
