...
| Documentation | Inspected, findings identified | `documentation/reference/FUNCTIONAL_SPEC.md`, `documentation/reference/readme/README.md`, `PROJECT_MANIFEST.md`, `documentation/reference/OPTIMIZATION_REFACTOR_STATUS.md` | Documentation | DOC-201 – DOC-205 | Not every one of the ~30 documents in `documentation/` was read in full |
...
- **Inspection status:** inspected · **Verification status:** **confirmed**
- **Evidence:** `documentation/reference/FUNCTIONAL_SPEC.md:3-5` ("Generated: 2026-08-29... describes the system as the code currently behaves"); zero matches for PHASE_VARYING, Monarch, financial_trends_reporter, or no_annualize; §4.5 (lines 149-153) still enumerates exactly 4 Roth conversion strategies, not the current 5.
...
- **DOC-202 — The shipped end-user README still reads "v11" after the v12 version bump** (Documentation, high confidence). `documentation/reference/readme/README.md:1`, `PROJECT_MANIFEST.md:1` were not in `bump_version.py`'s hardened replace-set. Recommend removing the version number from the README heading entirely (the app already reports its own version) and adding `PROJECT_MANIFEST.md` to the bump tool's target list.
...
3. A golden-master regeneration reflects the corrected SS claim-age logic, with a changelog entry per `documentation/reference/GOLDEN_MASTER_CHANGELOG.md`'s existing convention (W4-3).
4. The build backup archive, freshly generated, contains none of the three excluded paths (W4-5).
5. `documentation/reference/FUNCTIONAL_SPEC.md`'s generation date is not older than the newest commit touching a public engine surface (W4-8), verified by the new staleness guard.
...
- **No runtime validation was performed.** No tests were executed, no build was run, no browser session was opened. All findings rest on static file:line evidence or narrative/commit-history evidence. `documentation/archive/reports/SYSTEM_REVIEW_2026-08-31.md`'s own findings that *were* runtime-validated (e.g. its MC engine work) are treated here as prior evidence, not re-validated.
