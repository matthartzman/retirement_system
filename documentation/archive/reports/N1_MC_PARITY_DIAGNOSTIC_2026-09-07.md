...
**Date:** 2026-09-07. **Author:** Claude (session `session_01G3EV1KvMjaBAbgVnzJxYmT`), continuing Wave 4 item W4-2 from `documentation/archive/reports/WAVE4_IMPLEMENTATION_PLAN_2026-09-07.md`, after PR #92 ruled out the system review's two named suspects (home-equity contingency, survivor economics — see PR #92's commit message) without finding a positive cause.
...
## Follow-up attempt 5 (same day) — Finding 7: RMD has no balance-dependent recomputation, implemented, tested, made things WORSE

Chased the "open question" above (something changes TOTAL liquid trajectory) directly. Confirmed `_mc_vectorized_projection` (~480 lines) contains **zero** mentions of RMD anywhere — the vectorized engine has no RMD mechanism at all. It only ever draws pretax to cover that year's tier-tagged spend + tax, replayed from the deterministic baseline and scaled for inflation. RMD is a *mandatory* withdrawal (`= this year's actual balance / an age-based divisor`) the real household must take every year past RMD age regardless of spending need — deterministic_engine.py recomputes it fresh each year from the real, current balance (`compute_rmds`, already exposed as `row['rmd_total']`). Quantified the effect directly: by 2050, vectorized's own (already-diverged) pretax balance of $4.49M implies a required RMD of **$327,685**, while vectorized was actually only drawing the baseline-replay figure (~$88K-scale) — a ~3.7x under-draw, compounding every year (bigger unrequired balance → same fixed RMD → balance grows even faster → gap widens further). This is a clean, mechanically confident finding: no tax-bracket ambiguity, pure balance/age math, using state (`balances['pretax']`) the vectorized engine already tracks live per path.

**Implemented in full**: exposed `row['rmd_total']` (already existed) plus a new `prior_pretax_nw` series (the balance RMD is actually computed against) through `_mc_row_bucket_flows`/`_mc_effective_row_flows`; derived a per-year "implied divisor" from the baseline's own `(prior_pretax_nw, rmd_total)` pair (only their *ratio* reused, not either figure alone) and applied it to each path's own current `balances['pretax']`; forced that mandatory RMD out of pretax unconditionally before the tier cascade runs; pooled its after-tax proceeds with `income_funding` to offset that year's need; reinvested any RMD proceeds beyond need into taxable (mirroring real practice — RMD dollars can't be recycled back into pretax/Roth); added the RMD's tax increment (beyond what the baseline's `total_tax` already priced) to `'other'`. Combined with the Findings 2-4 tax-cascade fix as the foundation.

**Deterministic-harness result: `pretax` converges dramatically** — the 2056 gap, which was in the millions in every prior attempt, dropped to near-zero (both engines correctly draining pretax to ~$0 by end of plan, matching real RMD-driven depletion). **But `taxable` now massively overshoots** (2056: scalar $554,930 vs. vectorized $6.1M) — the household-level "blended divisor" approximation doesn't track a shifting per-spouse RMD composition over decades (this codebase tracks pretax as ONE combined bucket, not per-owner, so a static single-year blended ratio can't stay accurate as each spouse's own share and own age-based divisor diverge over a 30-year horizon).

**Stochastic parity-test result: WORSE — 33.5pp** (vectorized success rate 33.0% vs exact_scalar's 66.5%), up from 27.0pp with Findings 2-4 alone. **Reverted immediately** — no code shipped.

**This is the third distinct attempt this session where a mechanically well-justified, carefully-implemented fix moved the target metric the wrong direction** (Finding 2 alone: 8pp→worse when combined with 1; the first combined tax attempt: 40.5pp; now this: 27.0pp→33.5pp). That pattern is no longer noise — it is the actual, load-bearing conclusion of this diagnostic:

**Every fix attempted here is an *approximation* to what the real engine computes exactly per path.** `exact_scalar` agrees closely with the deterministic engine by construction — it reruns `project()`, the real per-path/per-year tax-and-RMD engine, once per simulated path. `vectorized` exists specifically to AVOID that per-path recomputation for speed, replaying one deterministic baseline trajectory (scaled for inflation, cut fraction, and now — attempted — bucket routing and RMD ratio) across every path instead. Every attempted fix in this diagnostic tried to patch that approximation closer to exact behavior with a cheap, blended heuristic, and each one that touched a real dollar amount (not just bucket sourcing) made the AGGREGATE metric worse or, at best, only partially better, because the heuristic's own error is not small relative to the gap it's trying to close. Findings 2-4 (fixing an outright double-count/double-drop of the tax-funding need, not an approximation refinement) is the only attempt that reliably helped, and even it alone is insufficient.

---

## Recommended next steps (final)

1. **This gap is very likely not closable with further heuristic cascade/dollar-figure patches.** Five follow-up attempts, each backed by hard test evidence, converge on this: closing it for real needs either (a) genuine per-path recomputation of tax and RMD in the vectorized engine — which likely means tracking pretax per-owner (not one combined bucket) and running real bracket/RMD math per path per year, at that point approaching `exact_scalar`'s own cost and defeating vectorization's purpose — or (b) accepting the vectorized engine as a disclosed, bounded-tolerance approximation (the original review's own Option 3: surface `mc_approximation_status` and a "±Xpp approximate" band on the KPI tile/workbook) rather than chasing exact parity. This is a product/architecture decision, not an engineering task, and belongs with whoever owns that tradeoff.
2. If (a) is chosen anyway: split the vectorized engine's `'pretax'` bucket into per-owner sub-balances (needed for both correct RMD divisors and correct bracket-ceiling AGI per spouse), and give it genuine per-path bracket/tax computation (not a blended baseline figure) — a substantial, multi-week-scale engineering project, not a follow-up patch.
3. If (b) is chosen: Findings 2-4 (the tax double-count/double-drop fix) should still land on its own merits — real, verified, safe, an actual bug fix independent of the parity question — paired with re-tightening or re-documenting the tolerance gate honestly (do not silently leave it wide without disclosure).
4. Finding 1 (HSA reorder), Finding 6 (bracket-capped pretax), and Finding 7 (RMD forcing) are each independently real, correctly implemented, and separately valuable for per-account/reporting accuracy and realism — but none of them reliably improves the success-rate parity metric alone, and two actively made it worse. Do not re-attempt any of them as a fix FOR THIS metric without a genuinely new hypothesis, tested exactly as rigorously as this diagnostic has (implement, run the 200-sim test, read the actual number) — five consecutive attempts at "this should obviously help" have taught that guessing the aggregate effect from the mechanism alone does not work in this system.
5. Re-run `tests/test_monte_carlo_default_engine_mode.py::test_exact_scalar_oracle_agrees_with_vectorized_default_within_tolerance` (200 sims, seconds) after every future step — non-negotiable, per point 4.
6. Golden-master regen and a `documentation/reference/GOLDEN_MASTER_CHANGELOG.md` entry are required once anything here actually ships, per this repo's standing discipline (`documentation/reference/CLAUDE.md`) — none of the six attempts here reached that point.
7. Planner sign-off recommended before shipping anything from this diagnostic, matching the review's own precedent for N2/N4 (financial-domain correctness changes that move simulated outcomes) — and, given point 1, likely a planner/architecture conversation about which of (a) or (b) this product should actually pursue, before more engineering time goes into either.
