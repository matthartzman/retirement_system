...
**Source:** `documentation/archive/reports/SYSTEM_REVIEW_2026-09-07.md`, §9 Wave 4 (W4-1..W4-5). This document turns that table into an executable plan: exact files, exact changes, verification steps, sequencing, and per-item model/effort sizing.
...
3. If affected: regenerate via `py -3.14 tools/regen_golden_master.py regen --reason "N2: SS claim-age month precision fix"` (per `documentation/reference/GOLDEN_MASTER_RECOVERY_RUNBOOK.md` — never hand-edit `PINNED_TERMINAL_NW`/`PINNED_LIFETIME_TAX`).
4. Add an entry to `documentation/reference/GOLDEN_MASTER_CHANGELOG.md` describing this as a **correction** (per the review's planner sign-off in §14), not silent drift.
...
**Changelog / status-record updates required by the review itself (§7, process fix):** none of Wave 4's items require a Wave 2/3 status-record correction (that's W4-9, Wave 5) — but `documentation/reference/GOLDEN_MASTER_CHANGELOG.md` does need the W4-3 entry per §2.3 step 4 above if the fixture is affected.
