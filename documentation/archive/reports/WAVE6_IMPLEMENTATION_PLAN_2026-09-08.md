...
**Date:** 2026-09-08. **Source:** `documentation/archive/reports/SYSTEM_REVIEW_2026-09-07.md`, the findings Wave 5
left explicitly unscheduled (ARC-4, ARC-6, ARC-7, DOC-204, DOC-205, N9, QUA-302, QUA-303, SEC-4, SEC-5,
UX-107) plus `documentation/archive/reports/WAVE_2_3_STATUS_CORRECTIONS_2026-09-07.md`'s re-scoped architecture
...
- **Effort:** S. **Risk:** Low, but touches a golden-master-adjacent fixture — full suite before calling this
  one done, per `documentation/reference/CLAUDE.md`'s fixture-change rule.

### W6-5 — No end-user documentation for Monarch auto-update or the trends reporter
- **Finding:** DOC-204 (Medium confidence).
- **What:** `documentation/reference/readme/README.md` (25 lines) mentions neither subsystem at all; the only existing
  docs (`documentation/archive/superpowers/plans/2026-09-02-monarch-autoupdate-reporting-plan.md`,
  `documentation/archive/superpowers/specs/2026-09-02-monarch-autoupdate-reporting-design.md`) are maintainer-facing design
...
- **What:** `documentation/` root holds at minimum `Final Optimization Implementation Plan.md`, `Final
  ...
  (`documentation/reference/OPTIMIZATION_DOCS_INDEX.md`) listing every doc in this cluster with its actual date, one-line
...
Per `documentation/reference/CLAUDE.md`: fast tier (`pytest tests/ -m "not slow" --tb=short -q`) after every item; full
