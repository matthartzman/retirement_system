...
> **Design correction #2 (2026-08-06, later the same session):** the first working version of `convert_dashboard.mjs` implemented the correction above by prefixing every one of the ~758 non-reassigned top-level functions with `export ` in place. Running the full test suite against it found ~20 regressions: several existing tests extract a slice of `dashboard.js` by searching for the exact literal text `'function someName('`, and `export ` landing 7 characters earlier shifts that boundary and breaks their extraction math (one already-fixed case, `setAppControls`, was in the plan's own Global Constraints section as a known risk; the other ~19 were not anticipated). A revised attempt used a single trailing `export { name1, name2, ... };` list statement instead (zero changes to the ~758 existing declaration lines) — but several *other* tests `eval()` the *entire* `dashboard.js` file as a plain non-module script to smoke-test its runtime behavior, and `export` in any form or position is a `SyntaxError` outside a real module. The final design drops `export` from this file entirely: nothing today does `import {x} from './dashboard.js'`, and `type="module"` on the script tag alone — not any `export` statement — is what makes the file's top-level bindings private (the actual goal). The window bridge is the entire backward-compat mechanism; `export` was purely cosmetic and only ever cost correctness. Verified against the full test suite: only the pre-existing (unrelated) baseline failures remain, plus the size-ratchet test Task 5 has not yet updated.
...
(Task 3 of documentation/archive/superpowers/plans/2026-08-06-dashboard-js-ast-module-conversion.md)
...
Expected: all pass (rerun serially without `-n` for any failure that looks like the documented Windows file-lock flake before treating it as real — see `documentation/reference/CLAUDE.md`'s Testing Discipline section).
