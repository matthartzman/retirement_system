...
**This is the real root cause Finding 5 was circling.** Fixing it means the vectorized engine needs, at minimum: a per-path/per-year AGI estimate, the same `bracket_top_24`/`irmaa_threshold` inputs the deterministic engine uses, and a genuine two-pass (capped, then taxable, then uncapped) cascade in place of today's single ordered walk — a materially larger change than anything attempted in this diagnostic so far. No code was written for this finding; it was traced and verified by reading `withdraw_pretax_elective` directly, not by another patch-and-test cycle.

---

## Recommended next steps (revised again)

1. Land the tax-double-count fix from follow-up attempt 2 (`other_nominal = total_tax`, income-netted once, no double gross-up anywhere) as its own step once Finding 6's bracket-capped withdrawal structure is also addressed — landing it alone, per the aggregate-test runs above, is real progress (40.5pp→27.0pp) but not yet passing, and this is financial-outcome code that shouldn't ship partially fixed.
2. **Finding 6 is the design decision that actually needs to happen before N1 can close**: give the vectorized engine a genuine two-pass pretax cascade (capped by a per-path AGI/bracket/IRMAA estimate, then taxable, then uncapped pretax), mirroring `withdraw_pretax_elective`'s real logic, rather than the current single ordered bucket walk. This is materially larger than a cascade-order patch and belongs with whoever owns the vectorized engine's design tradeoffs — likely the same person who should decide the per-path-tax question raised in follow-up attempt 1.
3. Finding 1 (HSA reorder) is still real and independently correct, but empirically makes the aggregate metric slightly worse every time it's been tested (3.5pp→8pp alone; 27.0pp→30.0pp atop the tax fix) — land it separately from the tax fixes and re-verify its aggregate effect once Findings 2-6 are otherwise resolved, rather than assuming "correct" implies "improves this metric."
4. Re-run `tests/test_monte_carlo_default_engine_mode.py::test_exact_scalar_oracle_agrees_with_vectorized_default_within_tolerance` (200 sims, seconds) after every step, exactly as this diagnostic has been doing — every attempt so far produced a different, informative number; guessing at the aggregate effect has not worked once.
...
7. Planner sign-off recommended before shipping, matching the review's own precedent for N2/N4 (financial-domain correctness changes that move simulated outcomes) — doubly so now that the scope includes a genuine bracket-aware withdrawal-cascade redesign, not just a tax-figure fix.
