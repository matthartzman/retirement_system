...
MAX_SHARPE_RECOMMENDATION_COMMENT = (
    ...
    "risk-aversion utility. The sleeve is scoped to core equity classes plus commodities -- REITs, "
    "Managed Futures, and Private Credit are excluded, since guaranteed income and home equity already "
    "fill the fixed-income/real-estate role elsewhere in this recommendation. It does not itself "
    "re-optimize the equity/bond/cash split or the fixed-income sub-class ladder."
)
TANGENCY_RECOMMENDATION_COMMENT = (
    "Pure tangency solves for the single long-only portfolio, across the enabled core equity classes "
    "plus commodities, with the highest Sharpe ratio -- with no risk-tolerance ceiling, glide path, or "
    "guaranteed-income/home-equity coverage overlay applied. Bonds, REITs, cash, and other alternatives "
    "are excluded from consideration entirely (not just left uncovered): guaranteed income and home "
    "equity already fill that role elsewhere in the plan, so this answers a narrower question -- given "
    "that, what is the best risk-adjusted mix of the liquid growth dollars? This is a textbook "
    "mean-variance tangency portfolio, not a risk-appropriate recommendation: because it freely trades "
    "off risk for Sharpe within that universe, it can concentrate heavily in a single equity class. "
