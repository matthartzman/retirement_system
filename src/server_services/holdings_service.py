...
def read_hsa_schedule(*, base_dir: Path, workspace_id: str, client_id: str, db_path: Path) -> dict[str, Any]:
    """Mirrors read_liabilities exactly. client_hsa_schedule.csv was registered
    in PLAN_DATA_CSV_FILES (blank-plan reset, workbook bundling) and had a
    ...
    if p.exists():
        return {"source": "workspace_file", "path": str(p), "content": None, "content_type": "text/csv"}
    content = get_client_file("client_hsa_schedule.csv", workspace_id, client_id, db_path)
    if content is not None:
        return {"source": "sqlite_client_file", "path": "", "content": content, "content_type": "text/csv"}
    return {"source": "empty_template", "path": "", "content": EMPTY_HSA_SCHEDULE_CSV, "content_type": "text/csv"}
