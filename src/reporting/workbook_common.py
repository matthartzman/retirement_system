...
# Wave 5 ARC-5 (system review 2026-09-07): this used to be
# `[name for name in globals() if not name.startswith("__")]`, which re-exported
# every stdlib/openpyxl name this module merely imports for its own use (csv,
# math, random, sys, traceback, json, Border, Side, xl_numbers, ...) alongside
# its genuine public surface. Every real consumer already does an explicit
# `from .workbook_common import (name, ...)` (grep `workbook_common import \(`
# under src/reporting/), so `__all__` had no actual effect on any importer --
# but a wildcard-import consumer added later would have silently inherited
# that entire junk-drawer surface. This list is exactly the names those
# explicit imports draw on today; add a name here only when a new consumer
# actually imports it.
__all__ = [
    "ASSET_CLASS_RETURNS",
    "Alignment",
