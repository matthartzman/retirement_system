...
def build_sheet10(ws, c, rows):
    ...
    # The remaining pair-evaluation machinery (monte_carlo, run_scenario,
    # _roth_discount_rate, LCV_FEASIBILITY_GATE_THRESHOLD, the after-tax
    # terminal estimator) moved with the pair evaluator into
    # sheets_strategy_pair_worker.py; only the two names this function still
    # calls directly are imported here.
    ...
    def _safe_project_pair(h_age, w_age, h_mort_age=None, w_mort_age=None, skip_mc=False):
        # Thin adapter over the module-level (picklable) worker so the serial
        # and the process-pool paths share one implementation. The pair logic
        # itself -- including every comment documenting its root-caused
        # historical bugs -- now lives in sheets_strategy_pair_worker.py.
