...
def _synthetic_zero_income_setup(n_years: int, total_spend: float, tiers: dict):
    ...
    requested withdrawal, deterministically, in every year. This avoids
    depending on the real client_data.csv fixture's income streams (SS,
    pension, wages), which mask a shortfall via _account_deposits regardless
    of how large spend_base is set -- a pre-existing vectorized-engine
    characteristic unrelated to this test.
    """
    plan_start = 2030
    ...
    base_rows = [
        {
            "year": y,
            "_account_withdrawals": {"acct1": total_spend},
...
def test_small_shortfall_is_absorbed_entirely_by_discretionary():
    ...
    c, base_rows, returns, inflation_paths, max_death_years = _synthetic_zero_income_setup(1, 5000.0, tiers)
...
def test_large_shortfall_spills_into_essential():
    tiers = {"essential": 40000.0, "important": 20000.0, "discretionary": 10000.0}
    # 40000 exceeds discretionary(10000) + important(20000) by 10000, which
    # must land in essential. Single year -- see comment above.
    c, base_rows, returns, inflation_paths, max_death_years = _synthetic_zero_income_setup(1, 40000.0, tiers)
    proj = _mc_vectorized_projection(c, base_rows, returns, inflation_paths, max_death_years)
