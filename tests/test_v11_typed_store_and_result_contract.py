...
def test_item_7_stage_pipeline_emits_stage_summaries():
    # Wave 2 item 2.2 (system review A2/A9): TaxAssessment is now registered
    # in STAGE_IMPLEMENTATIONS, delegating to the extracted tax_kernel module
    # (item 2.1) -- its event honestly reports "completed", not "inlined".
    # The other stages still have no independently callable implementation,
    # so their metrics still come from the one real engine_project() call.
    ...
    assert any(e.stage == 'RMDs' and e.event_type == 'inlined' for e in result.events)
