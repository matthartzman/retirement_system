...
def test_scenario_home_value_routes_to_home_sale_group_not_other_scenario_group():
    ...
    assert "const other=rs.filter(r=>!rowIsEconomyScenario(r)&&!homeSale.includes(r)" in js
...
def test_money_like_scenario_labels_get_currency_formatting_even_without_units():
    ...
    assert "'basis','proceeds'" in js
    assert "'taxes','tax','exclusion'" in js
    currency_check = "if(units.includes('$')||u.includes('usd')||u.includes('dollar')"
    number_check = "if(['year','integer','int','number','numeric'].includes(type))return 'number'"
...
def test_model_heard_includes_current_home_value():
    ...
    assert "Current Home Value is '+mhMoney(home.current_home_value)" in js
