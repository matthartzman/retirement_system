...
def _chart_root(name: str) -> ET.Element:
    with zipfile.ZipFile(WORKBOOK) as zf:
        return ET.fromstring(zf.read(f"xl/charts/{name}"))


def _pie_labels_and_values(chart_name: str) -> tuple[list[str], list[float], dict[str, str]]:
    root = _chart_root(chart_name)
...
def test_allocation_pies_do_not_show_series_name_or_zero_slices():
    for chart_name in ("chart5.xml", "chart6.xml"):
        labels, values, flags = _pie_labels_and_values(chart_name)
    ...
    current_labels, _, _ = _pie_labels_and_values("chart5.xml")
    target_labels, _, _ = _pie_labels_and_values("chart6.xml")
