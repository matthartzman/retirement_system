    ...
    (taxonomyData || []).forEach(function (typeData) {
      ...
      (typeData.groups || []).forEach(function (grp) {
        ...
        (grp.categories || []).forEach(function (cat) {
          html += `<tr><td><code class="small">${esc(cat.id)}</code></td><td>${esc(cat.label)}</td><td class="small">${esc(cat.notes || "")}</td><td>${deleteIconBtn(`deleteTaxonomyCat('${esc(cat.id)}','${esc(cat.label).replace(/'/g, "\\'")}')`)}</td></tr>`;
        });
    ...
    rules.forEach(function (rule, i) {
      ...
      if (current && !catIds.includes(current))
        opts =
          ...
          opts;
      html += `<tr><td><input value="${esc(rule.keyword)}" oninput="updateMappingRule(${i},'keyword',this.value)" style="width:160px"></td><td><select onchange="updateMappingRule(${i},'match_field',this.value)"><option value="category"${rule.match_field === "category" ? " selected" : ""}>Category text</option><option value="merchant"${rule.match_field === "merchant" ? " selected" : ""}>Merchant text</option></select></td><td style="width:64px;text-align:center"><input type="checkbox" ${rule.exact ? "checked" : ""} onchange="updateMappingRule(${i},'exact',this.checked)"></td><td><select onchange="updateMappingRule(${i},'category_id',this.value)" style="min-width:260px"><option value="" ${current ? "" : "selected"}>Select category…</option>${opts}</select></td><td><input type="number" value="${rule.priority || 50}" oninput="updateMappingRule(${i},'priority',parseInt(this.value)||50)" style="width:70px"></td><td style="width:64px;white-space:nowrap">${deleteIconBtn(`deleteMappingRule(${i})`)}</td></tr>`;
    });
  }
    ...
    (typeData.groups || []).forEach(function (grp) {
      ...
      if (gmode === "summary") {
        ...
        (grp.categories || []).forEach(function (cat) {
          ...
          const cNoAnnualizeRaw = b.no_annualize || "";
          const cIsNoAnnualize =
            cNoAnnualizeRaw === "TRUE"
              ? true
