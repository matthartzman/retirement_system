    ...
    (typeData.groups || []).forEach(function (grp) {
      ...
      if (gmode === "summary") {
        ...
        const gYearFields = ["Travel", "Large Discretionary"].includes(tt)
          ...
          : "";
        const gNoAnnualize = (taxBudget[gk] || {}).no_annualize || "";
        const gAnnualizeField = `<label class="small">Annualize&nbsp;</label><select ${readOnlyRef ? "disabled " : ""}onchange="updateTaxBudget('${esc(gk)}','no_annualize',this.value)" style="width:150px" title="Applies to every category in this group. Use for lumpy, one-or-two-installment-a-year spending (e.g. real estate taxes) where a partial-year run rate would badly misstate the full-year total."><option value=""${gNoAnnualize === "" ? " selected" : ""}>Default</option><option value="FALSE"${gNoAnnualize === "FALSE" ? " selected" : ""}>Normally (run-rate)</option><option value="TRUE"${gNoAnnualize === "TRUE" ? " selected" : ""}>Don't annualize (lumpy)</option></select> `;
        html += `<div class="table-actions"><label class="small">Group budget / yr&nbsp;</label><input ${readOnlyRef ? "disabled " : ""}type="text" class="budget-money-input" value="${esc(budgetMoneyInputValue((taxBudget[gk] || {}).annual_budget))}" placeholder="${catSum > 0 ? dollars0(catSum) : "$0"}" onfocus="focusBudgetMoney(this)" oninput="updateTaxBudgetMoney('${esc(gk)}','annual_budget',this)" onblur="blurBudgetMoney(this)" style="width:140px"> ${gYearFields}${gAnnualizeField}<span class="small">category and line detail disabled — group number wins</span></div>`;
      } else {
