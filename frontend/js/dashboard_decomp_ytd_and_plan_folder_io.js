...
export function renderHsaSchedule() {
  ...
  rows.forEach((r, i) => {
    ...
    html +=
      ...
      `<td data-label="Actions">${deleteIconBtn(`deleteHsaScheduleYear(${i})`)}</td>` +
      `</tr>`;
  });
  html += `</tbody></table></div></div>`;
  return html;
