...
const STEPS = [
  ...
  },
  {
    ...
    group: "Settings",
    title: "Settings",
    desc: "Everyday settings, backup, pricing, report readiness, and advanced maintenance.",
    intro:
      "Use Normal Settings for routine work. Open Advanced Maintenance only for diagnostics, reference data, or recovery tools.",
...
function renderMaxSharpeAllocationPanel() {
  return `<div class="holdings"><h3 class="group-title">Max-Sharpe (risk-budgeted) recommendation active</h3><div class="section-note">Keeps the same risk level as the allocation optimizer recommendation (risk tolerance/auto risk score, glide path, guaranteed-income/home-equity coverage), but chooses the equity sleeve's sub-class weights to maximize the sleeve's own Sharpe ratio (return in excess of the risk-free rate, per unit of volatility) instead of a fixed risk-aversion utility. The sleeve is scoped to core equity classes plus commodities &mdash; REITs, Managed Futures, and Private Credit are excluded, since guaranteed income and home equity already fill the fixed-income/real-estate role elsewhere in this recommendation. It does not itself re-optimize the equity/bond/cash split, and does not support a manual override.</div>${allocationOptimizerRecommendationHtml()}</div>`;
}
function renderTangencyAllocationPanel() {
  return `<div class="holdings"><h3 class="group-title">Pure tangency recommendation active</h3><div class="section-note warn"><b>No risk budget:</b> this solves for the single long-only portfolio, across the enabled core equity classes plus commodities, with the highest possible Sharpe ratio. Bonds, REITs, cash, and other alternatives are excluded entirely (not just left uncovered) &mdash; guaranteed income and home equity already fill that role elsewhere in the plan, so this answers a narrower question: given that, what's the best risk-adjusted mix of the liquid growth dollars? Risk tolerance and glide path are not applied, and it does not support a manual override. It can concentrate heavily in a single equity class depending on the configured capital-market assumptions &mdash; review it as an analytical reference before using it to drive the plan.</div></div>`;
}
