...
(function(){
  ...
  function escCtx(ctx,v){return ctx&&ctx.esc?ctx.esc(v):esc(v)}
  ...
  function workbookSheetSectionInt(name){var m=String(name||'').match(/^(\d+)/);return m?m[1]:null}
  function isExcelTabSheet(s){return s.source==='excel_parser_fallback'||/^\d+[A-Za-z]/.test(String(s.name||''))}

  // ---- Detail cell helpers ----
  function workbookCellSearchText(row){return ((row?.cells)||[]).map(c=>String(c.display??c.value??'')).join(' ').toLowerCase()}
  function detailSectionMatches(section,q){if(!q)return true;const title=String(section?.title||'').toLowerCase();if(title.includes(q))return true;return ((section?.rows)||[]).some(r=>workbookCellSearchText(r).includes(q))}
  function filteredDetailRows(section,q){const rows=section?.rows||[];if(!q)return rows;return rows.filter(r=>workbookCellSearchText(r).includes(q)||String(section?.title||'').toLowerCase().includes(q))}
  function detailFallbackHeader(i,maxCells){const base=['Description','Value','Detail','Scenario','Year','Amount','Result','Notes'];return base[i]||`Detail ${i+1}`}
  const DETAIL_MONEY_TERMS=['amount','balance','value','price','basis','sale','gross','income','spending','expense','cost','payment','mortgage','tax','taxes','premium','benefit','withdrawal','contribution','distribution','cash','net worth','nw','ending nw','terminal nw','delta vs base','principal','interest','dividend','gain','capital gain','ltcg','salary','rent','reserve','proceeds','equity'];
  const DETAIL_PERCENT_TERMS=['percent','percentage','pct','rate','return','yield','growth','inflation','cola','success probability','probability','volatility','allocation'];
  function detailNumericValue(cell){if(cell&&typeof cell.value==='number'&&Number.isFinite(cell.value))return cell.value;const raw=String(cell?.value??cell?.display??'').trim();if(!raw||/[a-zA-Z]/.test(raw.replace(/[KMB%$]/g,'')))return null;const neg=/^\(.*\)$/.test(raw)||/^\s*-/.test(raw);let mult=1;let cleaned=raw.replace(/[$,%()\s]/g,'');if(/[kK]$/.test(cleaned)){mult=1000;cleaned=cleaned.slice(0,-1)}else if(/[mM]$/.test(cleaned)){mult=1000000;cleaned=cleaned.slice(0,-1)}else if(/[bB]$/.test(cleaned)){mult=1000000000;cleaned=cleaned.slice(0,-1)}cleaned=cleaned.replace(/,/g,'');const n=Number(cleaned);if(!Number.isFinite(n))return null;return (neg?-Math.abs(n):n)*mult}
  function detailText(cell){return String(cell?.display??cell?.value??'').trim()}
  function detailRowText(row){return ((row?.cells)||[]).map(detailText).filter(Boolean).join(' ')}
  function detailIsYearLike(n,context){return Number.isInteger(n)&&n>=1900&&n<=2200&&(/(^|[^a-z])year([^a-z]|$)/.test(context)||Math.abs(n)<2300)}
  function detailIsAgeContext(context){return /(^|[^a-z])age([^a-z]|$)/.test(context)||/(^|\s)[hw]\s*age($|\s)/.test(context)}
  function detailCurrencyK(n){if(!Number.isFinite(n))return '';const neg=n<0;const abs=Math.abs(n);if(Math.round(abs/1000)===0)return (neg?'-':'')+'$'+Math.round(abs);const k=Math.round(abs/1000);return (neg?'-':'')+'$'+k.toLocaleString(undefined,{maximumFractionDigits:0})+'K'}
  function detailPercentDisplay(n){if(!Number.isFinite(n))return '';const pct=Math.abs(n)<=1?n*100:n;return pct.toLocaleString(undefined,{maximumFractionDigits:0,minimumFractionDigits:0})+'%'}
  function detailIntegerDisplay(n){if(!Number.isFinite(n))return '';return String(Math.round(n))}
  function detailNumberDisplay(n){if(!Number.isFinite(n))return '';const decimals=Math.abs(n)<100&&Math.abs(n%1)>0?2:0;return n.toLocaleString(undefined,{maximumFractionDigits:decimals,minimumFractionDigits:0})}
  function detailRowHasMostlyBlankLeadingCells(cells){return cells.length&&cells.slice(0,2).every(c=>!detailText(c))}
  function detailCellIsYearText(text){const n=Number(String(text||'').replace(/\.0$/,''));return Number.isInteger(n)&&n>=1900&&n<=2200}
  function detailHeaderScore(row,next){const cells=row?.cells||[];const nonblank=cells.map(detailText).filter(Boolean);if(nonblank.length<2)return -1;const textCount=nonblank.filter(x=>/[A-Za-z]/.test(x)).length;const yearCount=nonblank.filter(detailCellIsYearText).length;const nextNonblank=((next?.cells)||[]).map(detailText).filter(Boolean).length;let score=nonblank.length+(nextNonblank>=2?4:0)+(textCount?3:0)+(yearCount>=2?5:0);if(nonblank.length===1)score-=8;if(detailRowHasMostlyBlankLeadingCells(cells)&&textCount<=1)score-=2;return score}
  function detailHeaderRowIndex(section,rows){if(!rows.length)return -1;let best=-1,bestScore=-1;const limit=Math.min(rows.length,18);for(let i=0;i<limit;i++){const score=detailHeaderScore(rows[i],rows[i+1]);if(score>bestScore){bestScore=score;best=i}}return bestScore>=7?best:-1}
  function detailIdentifierRowIndex(rows,headerIdx){if(headerIdx<=0)return -1;for(let i=headerIdx-1;i>=0;i--){const cells=rows[i]?.cells||[];const vals=cells.map(detailText);const nonblank=vals.filter(Boolean);if(!nonblank.length)continue;const text=nonblank.join(' ').toLowerCase();if(text.includes('identifier')||nonblank.length>=2||cells.length>3)return i;if(nonblank.length===1&&i===0&&headerIdx===1)return i}return -1}
  function detailCellContext(section,row,cellIndex,headers){const parts=[detailText(headers[cellIndex]||{}),detailText((row?.cells||[])[0]||{}),detailText((row?.cells||[])[1]||{})];return norm(parts.join(' '))}
  function detailTermInContext(context,term){const t=norm(term);return new RegExp('(^|[^a-z0-9])'+t.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'([^a-z0-9]|$)').test(context)}
  function detailCellKind(section,row,cellIndex,headers){const c=(row?.cells||[])[cellIndex]||{};const kind=String(c.kind||'').toLowerCase();const n=detailNumericValue(c);const context=detailCellContext(section,row,cellIndex,headers);if(n!==null&&detailIsYearLike(n,context))return 'year';if(n!==null&&detailIsAgeContext(context))return 'integer';if(['percent','date','text','boolean'].includes(kind))return kind;if(kind==='currency'&&n!==null)return 'currency';if(n===null)return 'text';if(DETAIL_PERCENT_TERMS.some(term=>detailTermInContext(context,term)))return 'percent';if(DETAIL_MONEY_TERMS.some(term=>detailTermInContext(context,term)))return 'currency';return 'number'}
  function detailCellDisplay(section,row,cellIndex,headers){const c=(row?.cells||[])[cellIndex]||{};const raw=detailText(c);const n=detailNumericValue(c);if(n===null)return raw;const kind=detailCellKind(section,row,cellIndex,headers);if(kind==='currency')return detailCurrencyK(n);if(kind==='percent')return detailPercentDisplay(n);if(kind==='year'||kind==='integer')return detailIntegerDisplay(n);return detailNumberDisplay(n)}
  function detailColumnGroupKey(section,groupIndex){return `${String(section?.title||'section').replace(/[^a-z0-9]+/gi,'_').slice(0,42)}_${Number(section?.start_row||0)}_${groupIndex}`}
  ...
  function detailColumnGroups(section,labels,pinned){const detail=[];labels.forEach((_,i)=>{if(!pinned.has(i))detail.push(i)});const yearCount=detail.filter(i=>detailCellIsYearText(String(labels[i]||''))).length;const gSize=yearCount>=Math.max(3,detail.length*0.5)?10:6;const groups=[];for(let i=0;i<detail.length;i+=gSize){const cols=detail.slice(i,i+gSize);groups.push({index:groups.length,cols,label:detailGroupLabel(section,labels,cols)})}return groups}
  ...
  function detailInferYLabel(max){if(max<=1.5)return 'Rate (%)';if(max<=150&&Number.isInteger(Math.round(max)))return 'Count';return 'Amount ($K)'}
  function detailChartColor(i){return ['#000000','#E69F00','#56B4E9','#009E73','#F0E442','#0072B2','#D55E00','#CC79A7'][i%8]}
  function detailChartValues(chart){return (chart?.series||[]).flatMap(s=>(s.values||[]).map(Number).filter(Number.isFinite))}
  function detailChartMaxStack(chart){const xs=chart?.x||[];let max=0;xs.forEach((_,i)=>{let total=0;(chart.series||[]).forEach(s=>{total+=Math.max(0,Number((s.values||[])[i])||0)});max=Math.max(max,total)});return max||1}
  function renderDetailStackedBarChart(chart){
    var xs=chart.x||[],series=chart.series||[],max=detailChartMaxStack(chart);
    var w=960,h=340,plotX=80,plotY=28,plotW=820,plotH=230,step=xs.length?plotW/xs.length:plotW,barW=Math.max(2,Math.min(14,step*.62));
    var niceVals=niceTickRange(max);var ticks=niceVals.map(function(v){return v/max});
    var yLabel=chart.y_label||detailInferYLabel(max);
    var svg='<svg viewBox="0 0 '+w+' '+h+'" role="img" aria-label="'+esc(chart.title||'Chart')+'">';
    ticks.forEach(function(t,i){var y=plotY+plotH-(t*plotH);var rawVal=niceVals[i];var label=max<=1.5?(rawVal*100).toFixed(0)+'%':max<=150?String(Math.round(rawVal)):detailCurrencyK(rawVal);svg+='<line x1="'+plotX+'" y1="'+y.toFixed(1)+'" x2="'+(plotX+plotW)+'" y2="'+y.toFixed(1)+'" stroke="#e8e2d5" stroke-width="1"/>';svg+='<text x="'+(plotX-6)+'" y="'+(y+4).toFixed(1)+'" text-anchor="end" class="detail-chart-tick">'+esc(label)+'</text>';});
    svg+='<line x1="'+plotX+'" y1="'+(plotY+plotH)+'" x2="'+(plotX+plotW)+'" y2="'+(plotY+plotH)+'" class="detail-axis"/>';
    svg+='<line x1="'+plotX+'" y1="'+plotY+'" x2="'+plotX+'" y2="'+(plotY+plotH)+'" class="detail-axis"/>';
    svg+='<text x="14" y="'+(plotY+plotH/2)+'" text-anchor="middle" class="detail-chart-tick" transform="rotate(-90 14 '+(plotY+plotH/2)+')">'+esc(yLabel)+'</text>';
    svg+='<text x="'+(plotX+plotW/2)+'" y="'+(h-4)+'" text-anchor="middle" class="detail-chart-tick">Year</text>';
    xs.forEach(function(x,i){var y=plotY+plotH;series.forEach(function(s,si){var val=Math.max(0,Number((s.values||[])[i])||0);var rh=val/max*plotH;if(rh>0.5){svg+='<rect x="'+(plotX+i*step+(step-barW)/2).toFixed(1)+'" y="'+(y-rh).toFixed(1)+'" width="'+barW.toFixed(1)+'" height="'+rh.toFixed(1)+'" fill="'+detailChartColor(si)+'"><title>'+esc(s.label)+' '+esc(x)+': '+esc(detailCurrencyK(val))+'</title></rect>'}y-=rh;});if(i%Math.max(1,Math.ceil(xs.length/8))===0)svg+='<text x="'+(plotX+i*step+step/2).toFixed(1)+'" y="'+(plotY+plotH+16)+'" text-anchor="middle" class="detail-chart-tick">'+esc(String(x))+'</text>';});
    svg+='</svg>';
    return svg+renderDetailChartLegend(series);
  }
  function renderDetailLineChart(chart){
    var xs=chart.x||[],series=chart.series||[],vals=detailChartValues(chart);
    var max=Math.max.apply(null,vals.concat([1])),min=Math.min.apply(null,vals.concat([0]));
    var range=max-min||1,w=960,h=340,plotX=80,plotY=28,plotW=820,plotH=230;
    var niceVals=niceTickRange(max);var ticks=niceVals.map(function(v){return v/max});
    var yLabel=chart.y_label||detailInferYLabel(max);
    var svg='<svg viewBox="0 0 '+w+' '+h+'" role="img" aria-label="'+esc(chart.title||'Chart')+'">';
    ticks.forEach(function(t,i){var val=min+t*range;var y=plotY+plotH-t*plotH;var rawVal=niceVals[i];var label=max<=1.5?(rawVal*100).toFixed(0)+'%':max<=150?String(Math.round(rawVal)):detailCurrencyK(rawVal);svg+='<line x1="'+plotX+'" y1="'+y.toFixed(1)+'" x2="'+(plotX+plotW)+'" y2="'+y.toFixed(1)+'" stroke="#e8e2d5" stroke-width="1"/>';svg+='<text x="'+(plotX-6)+'" y="'+(y+4).toFixed(1)+'" text-anchor="end" class="detail-chart-tick">'+esc(label)+'</text>';});
    svg+='<line x1="'+plotX+'" y1="'+(plotY+plotH)+'" x2="'+(plotX+plotW)+'" y2="'+(plotY+plotH)+'" class="detail-axis"/>';
    svg+='<line x1="'+plotX+'" y1="'+plotY+'" x2="'+plotX+'" y2="'+(plotY+plotH)+'" class="detail-axis"/>';
    svg+='<text x="14" y="'+(plotY+plotH/2)+'" text-anchor="middle" class="detail-chart-tick" transform="rotate(-90 14 '+(plotY+plotH/2)+')">'+esc(yLabel)+'</text>';
    svg+='<text x="'+(plotX+plotW/2)+'" y="'+(h-4)+'" text-anchor="middle" class="detail-chart-tick">Year</text>';
    series.forEach(function(s,si){var pts=(s.values||[]).map(function(v,i){var x=plotX+(xs.length<=1?0:i/(xs.length-1)*plotW);var y=plotY+plotH-((Number(v)||0)-min)/range*plotH;return x.toFixed(1)+','+Math.max(plotY,Math.min(plotY+plotH,y)).toFixed(1);}).join(' ');svg+='<polyline points="'+pts+'" fill="none" stroke="'+detailChartColor(si)+'" stroke-width="2.5"><title>'+esc(s.label)+'</title></polyline>';});
    xs.forEach(function(x,i){if(i%Math.max(1,Math.ceil(xs.length/8))===0)svg+='<text x="'+(plotX+(xs.length<=1?0:i/(xs.length-1)*plotW)).toFixed(1)+'" y="'+(plotY+plotH+16)+'" text-anchor="middle" class="detail-chart-tick">'+esc(String(x))+'</text>';});
    svg+='</svg>';
    return svg+renderDetailChartLegend(series);
  }
  function detailPiePath(cx,cy,r,start,end){const sx=cx+r*Math.cos(start),sy=cy+r*Math.sin(start),ex=cx+r*Math.cos(end),ey=cy+r*Math.sin(end),large=end-start>Math.PI?1:0;return `M ${cx} ${cy} L ${sx} ${sy} A ${r} ${r} 0 ${large} 1 ${ex} ${ey} Z`}
  function renderDetailPieChart(chart){const slices=(chart.slices||[]).filter(s=>Number(s.value)>0);const total=slices.reduce((n,s)=>n+Number(s.value||0),0)||1;let angle=-Math.PI/2;let svg='<svg viewBox="0 0 300 300" role="img" aria-label="'+esc(chart.title||'Chart')+'" style="width:100%;max-width:260px;flex-shrink:0">';slices.forEach((s,i)=>{const span=Number(s.value||0)/total*Math.PI*2;svg+=`<path d="${detailPiePath(150,150,105,angle,angle+span)}" fill="${detailChartColor(i)}"><title>${esc(s.label)}: ${esc(detailCurrencyK(Number(s.value||0)))} (${Math.round(Number(s.value||0)/total*100)}%)</title></path>`;angle+=span});svg+='</svg>';const legendItems=slices.map((s,i)=>`<div style="display:flex;align-items:center;gap:6px;font-size:12px;line-height:1.4"><i style="display:inline-block;width:12px;height:12px;min-width:12px;border-radius:2px;background:${detailChartColor(i)}"></i><span>${esc(s.label)} · ${Math.round(Number(s.value||0)/total*100)}%</span></div>`).join('');return `<div style="display:flex;flex-direction:column;align-items:center;gap:12px">${svg}<div style="display:flex;flex-direction:column;gap:5px;align-self:stretch">${legendItems}</div></div>`;}
  function renderDetailChartLegend(series){const items=(series||[]).slice(0,12);return `<div class="detail-chart-legend">${items.map((s,i)=>`<span><i style="background:${detailChartColor(i)}"></i>${esc(s.label||'Series')}</span>`).join('')}${(series||[]).length>items.length?`<span>+${(series||[]).length-items.length} more</span>`:''}</div>`}
  function renderDetailChartCard(ctx,chart){
    ...
    try{
      ...
      else if(chart.type==='bar'||chart.type==='stacked_bar'||chart.type==='stacked-bar')body=renderDetailStackedBarChart(chart);
      else if(!chart.type||KNOWN_TYPES.indexOf(chart.type)<0)body='<div class="chart-type-note">Chart type not yet supported: '+esc(chart.type||'unknown')+'</div>';
      else body=renderDetailStackedBarChart(chart);
    }catch(e){body='<div class="section-note">This chart could not be rendered in the UI. Use Download Workbook to view the Excel chart.</div>'}
    var cacheId=ctx&&ctx.cacheChart?ctx.cacheChart(body,chart.title||'Chart'):'nocache';
    return '<section class="detail-chart-card chart-expandable" onclick="openCachedChart(\''+cacheId+'\')" title="Click to expand">'+
      '<div class="chart-expand-hint">&#x2922; Expand</div>'+
      '<h4>'+esc(chart.title||'Chart')+'</h4>'+body+'</section>';
  }
  function renderChartDashboardSheet(ctx,sheet){
    const charts=[...(sheet.charts||[])].sort((a,b)=>{
      ...
      if(aEx&&!bEx)return -1;if(!aEx&&bEx)return 1;return 0;
    });
    let html=`<div class="detail-sheet-title"><h3>${esc(resultDisplayName(sheet.name))}</h3><span>${Number(charts.length)} charts</span></div>`;html+=`<div class="section-note detail-preview-note">${esc(sheet.chart_note||'Chart-only dashboard. Source ranges are hidden and not displayed in the explorer.')}</div>`;if(!charts.length){html+=`<div class="missing-list"><h3>No chart data available</h3><p>Use Download Workbook to view the embedded Excel chart dashboard.</p></div>`;return html}html+=`<div class="detail-chart-grid">${charts.map(c=>renderDetailChartCard(ctx,c)).join('')}</div>`;return html}

  // ---- Nav rendering ----
  function setDetailedResultsNavOpen(ctx,open){
    call(()=>ctx.setDetailedResultsNavOpenValue(!!open));
    try{if(window.localStorage)window.localStorage.setItem('retirementDetailedResultsNavOpen',open?'1':'0')}catch(_e){}
  }
  function renderDetailedResultsNav(ctx){
    ctx=ctx||{};
    const navOpen=!!call(ctx.getDetailedResultsNavOpen)||call(ctx.getActiveStep)==='detailed_results';
    ...
    const activeCls=call(ctx.getActiveStep)==='detailed_results'?'active':'';
    ...
    if(data&&data.success&&Array.isArray(data.sheets)){
      ...
      allSheets.forEach(function(sh){
        ...
        groups.push(g);groupMap[snum]=g;
      });
      allSheets.forEach(function(sh){
        if(!isWorkbookContentSheet(sh.name)&&!isWorkbookSectionDivider(sh.name))return;
        const snum=workbookSheetSectionInt(sh.name);
        if(!snum)return;
        if(!groupMap[snum]){const g={label:'Section '+snum,snum:snum,sheets:[]};groups.push(g);groupMap[snum]=g;}
        groupMap[snum].sheets.push(sh);
      });
      groups.sort(function(a,b){return Number(a.snum)-Number(b.snum);});
      groups.forEach(function(group){
        const catOpen=navOpen&&(call(ctx.getActiveStep)==='detailed_results'||group.sheets.some(function(sh){return sh.name===call(ctx.getActiveDetailedSheet);}));
        ...
        group.sheets.forEach(function(sheet){
          const cls=sheet.name===call(ctx.getActiveDetailedSheet)?'active':'';
      ...
      html+='<div class="detail-nav-note">Build outputs, then open this section to browse detailed results.</div>';
    }
    html+='</div></details>';
    return html;
  }
  ...
  function renderDetailedResults(ctx){
    ...
    const sheet=call(()=>ctx.detailedSheetByName(activeSheet))||((data.sheets||[])[0]);
    const q=String(call(ctx.getDetailResultsSearchText)||'').trim().toLowerCase();
    ...
    html+=`<div class="detail-results-toolbar"><label><b>Search this result sheet</b><input class="search" value="${escCtx(ctx,call(ctx.getDetailResultsSearchText)||'')}" placeholder="Filter rows within the selected result page..." oninput="detailResultsSearchText=this.value;renderMain()"></label><label><b>Sheet</b><select onchange="setDetailedResultSheet(this.value)">${(data.sheets||[]).filter(s=>isExcelTabSheet(s)).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''))).map(s=>`<option value="${escCtx(ctx,s.name)}" ${sheet&&s.name===sheet.name?'selected':''}>${escCtx(ctx,s.name)}</option>`).join('')}</select></label></div>`;
    if(!sheet){html+=`<div class="section-note">No workbook sheets were found.</div></div>`;return html;}
    ...
    if(!Array.isArray(sheet.sections)){
      if(!call(ctx.getDetailedResultSheetLoading)&&!call(ctx.getDetailedResultSheetError))setTimeout(()=>call(()=>ctx.loadDetailedResultSheet(sheet.name,false)),0);
    ...
    if(sheet.preview_note)html+=`<div class="section-note detail-preview-note">${escCtx(ctx,sheet.preview_note)}${sheet.truncated?' Showing a bounded preview so this sheet returns quickly.':''}</div>`;
    const sections=(sheet.sections||[]).filter(sec=>detailSectionMatches(sec,q));
    if(!sections.length)html+=`<div class="section-note">No rows in this sheet match the current search.</div>`;
    sections.forEach(function(sec,i){
      const rows=filteredDetailRows(sec,q);
      const sectionTitle=detailCleanSectionTitle(sec.title)||resultDisplayName(sheet.name)||'Details';
